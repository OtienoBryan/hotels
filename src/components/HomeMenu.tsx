...
const MenuPage = () => {
  return (
   <div className="p-4 lg:px-20 xl:px-40 h-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 bg-gray-100 opacity-70">
  {menu.map((category) => (
    <Link
      href={`/sub_menu/${category.slug}`}
      key={category.id}
      className="w-full h-34 lg:h-64 bg-cover rounded-md flex flex-col justify-end"
