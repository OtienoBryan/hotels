...
const CategoryPage = async ({ params }: Props) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 m-2 gap-4 text-indigo-950 mt-20">
  {products.map((item) => (
    <Link
    ...
    >
      ...
      <div className="w-2/3 flex flex-col justify-between ml-4">
        ...
        {/* Description */}
        <p className="text-xs lg:text-sm text-gray-600 mt-1 line-clamp-3">
          {item.desc}
        </p>

        {/* Price */}
        <h2 className="text-sm lg:text-base text-red-700">KES. {item.price} /-</h2>
